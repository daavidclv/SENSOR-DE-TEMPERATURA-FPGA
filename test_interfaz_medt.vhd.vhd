...
entity test_interfaz_medt is
end entity;

architecture test of test_interfaz_medt is
...
dut: entity work.interfaz_medt(estructural)  -- Completar nombre
     port map(
        clk            => clk,
        nRst           => nRst,
  --      tic            => tic,       -- ACTIVAMOS cada 3000 ciclos de clk
        SIO            => SDAT,      -- Línea de datos SI/O (entrada/salida)
        K1             => K1 , 
        K0             => K0, 
        CS             => CS,        
        SC             => CL       
...
-----SIMULACIOIN PULSADORES CON REBOTES-----------------
--
--process   
--begin
--  wait until clk'event and clk = '1';
...
-- wait until clk'event and clk = '1';
--    K1 <= '1';
-- wait for 11996*T_clk;  --Cada 3 medidas 
--                       
--end process;

--process     -- Simulación Pulsador
--begin
--  wait until clk'event and clk = '1';
--    K0 <= '1';
--   wait until clk'event and clk = '1';
...
-----GENERADOR SECUECNIA TEMP DE TEST --------------------
process  
